...
const Dashboard = () => {
  ...
  const getImageName = () => {
    fetch(`http://localhost:4000/api/auth/getImageName/${email}`)
      .then(res => res.json())
  ...
  const getCartData = () => {
    fetch('http://localhost:4000/api/auth/Cart/' + email)
      .then(res => res.json())
  ...
  const updateProfilePicFormHandler = async (e) => {
    ...
    try {
      const email = localStorage.getItem('email')
      const res =await axios.post("http://localhost:4000/api/auth/uploadprofilepic/" + email, formData, {
        headers: {
